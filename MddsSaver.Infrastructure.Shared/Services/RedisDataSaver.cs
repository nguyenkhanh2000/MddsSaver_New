...
{
    public class RedisDataSaver : IDataSaver
    {
        ...
        public RedisDataSaver(ILogger<RedisDataSaver> logger, IRedisRepository redisRepo, IRedisSentinelRepository redisSentinelRep, AppSetting appsetting)
        {
            _logger = logger;
                ...
                foreach (var msg in messages)
                {
                    ...
                    if (typeMsg == typeof(ESecurityDefinition)) // msg_d
                    {
                        ...
                        if (data.BoardID == "G1" && !d_dic_stockno.ContainsKey(data.Symbol))
                        {
                            ...
                            {
                                Key = _appSetting.Redis.Key_StockNo,
                                Value = stockno,
                    ...
                    if (typeMsg == typeof(EPrice))  // msg_X
                    {
                        ...
                        if (eP.BoardID == "G1" && eP.Side == null) // Khớp lệnh, OHLC
                        {
                            commands_Sentinel.Add(Create_LE_TKTT_Command(eP, Symbol));
                            commands_Sentinel.Add(Create_LS_Command(eP, Symbol));
                        }
                        else if (eP.BoardID == "G4" && eP.Side != null)
                        {
                            commands.Add(Create_PO_Command(eP, Symbol));
                        }
                        else if ((eP.BoardID == "T1" || eP.BoardID == "T4" || eP.BoardID == "T2" || eP.BoardID == "T3" || eP.BoardID == "T6" || eP.BoardID == "R1") /*&& eP.Side != null*/)
                        {
        ...
        private SortedSetAddCommand Create_LS_Command(EPrice eP, string Symbol)
        {
            try
            {
                ...
                if (checkDI) 
                {
                    strJsonC = JsonConvert.SerializeObject(new
                    {
                        ls_model.SQ,
                        ls_model.CN,
                        ls_model.MT,
                        MP = eP.MatchPrice, // Lấy giá trị DOUBLE gốc
                        ls_model.MQ,
                        ls_model.TQ,
                        ls_model.TV,
                        ls_model.SIDE
        ...
        private UpdateSortedSetByScoreCommand Create_LE_TKTT_Command(EPrice eP, string Symbol)
        {
            try
            {
                ...
                if (checkDI)
                {
                    // Nếu là DVX: ghi đè MP là double
                    strJsonC = JsonConvert.SerializeObject(new
                    {
                        le_tktt.MT,
                        MP = eP.MatchPrice, // Lấy giá trị DOUBLE gốc
